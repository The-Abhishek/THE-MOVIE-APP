...
sidebar <- dashboardSidebar(
  sidebarMenu(
    img(src = "http://tonystake.com/wp-content/uploads/2015/04/production-film-reel.png", height = 240, width = 230),
...
page <- dashboardBody(
  ...
  fluidRow(
    box(width = 12, DT::DTOutput("movies_table"), title = "All Selected Movies")
  )
)
...
server<-(function(input, output) {
  ...
  output$top_box_office_collections <- renderAmCharts({
    top_box_office_collections_plot <- filtered_data()
    top_box_office_collections_plot <- top_box_office_collections_plot[order(top_box_office_collections_plot$Revenue_in_Millions, decreasing = T), ]
    top_box_office_collections_plot <- head(top_box_office_collections_plot, 10)
    amBarplot(x = "Title", y = "Revenue_in_Millions", data = top_box_office_collections_plot,
              labelRotation = -45) 
  })
